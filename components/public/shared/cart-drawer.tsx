...
export function CartDrawer({ open, onClose }: CartDrawerProps) {
  const { cartLines, cartSubtotal, cartDiscount, updateCartQuantity, removeFromCart, db, locale } = useStore();
  const t = (ko: string, en: string) => (locale === "ko" ? ko : en);
  const [selectedSampleSlug, setSelectedSampleSlug] = useState("");
  ...
  const activeSampleSlug = selectedSampleSlug || db.products[0]?.slug || "";
  ...
  return (
    <section
      className="fixed inset-x-0 bottom-0 top-[var(--public-header-height)] z-[80] overflow-hidden"
      role="dialog"
      aria-modal="true"
      aria-label={t("장바구니 서랍", "Cart drawer")}
    >
      <button
        type="button"
        aria-label={t("장바구니 닫기", "Close cart drawer")}
        className="absolute inset-0 bg-black/30 backdrop-blur-md transition-opacity animate-[fadeIn_0.3s_ease-out_forwards]"
        onClick={onClose}
      />

      <aside className="absolute right-0 top-0 h-full w-full max-w-md bg-white dark:bg-[#2d1b20] shadow-2xl flex flex-col animate-[slideIn_0.4s_cubic-bezier(0.16,1,0.3,1)_forwards]">
        ...
        {cartLines.length === 0 ? (
          <div className="flex-1 px-6 py-8 grid place-items-center text-center">
            <div className="space-y-3">
              <h3 className="text-xl font-bold text-[#1b0e11] dark:text-white">
                {resolveText(CONTENT_COPY.cartEmptyTitle, locale)}
              </h3>
              <p className="text-sm text-[#974e60] dark:text-gray-400">
                {resolveText(CONTENT_COPY.cartEmptyBody, locale)}
              </p>
              <Link
                href="/shop"
                className="inline-flex h-11 items-center justify-center px-6 rounded-full bg-[#e6194c] text-white text-sm font-semibold hover:bg-[#cb1743] transition-colors"
                onClick={onClose}
              >
          ...
          <>
            <div className="flex-1 overflow-y-auto no-scrollbar px-6 py-2 space-y-8 bg-white dark:bg-[#2d1b20]">
                    ...
                    <div className="flex flex-1 flex-col justify-between py-1">
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <div className="flex items-center border border-[#f3e7ea] dark:border-[#4a3b3e] rounded-md h-7 w-fit bg-[#f8f6f6] dark:bg-[#211115] overflow-hidden">
                          ...
                          <span className="text-xs font-medium w-8 text-center tabular-nums text-[#1b0e11] dark:text-white">
                            {line.item.quantity}
                          </span>
                          <button
                            type="button"
              ...
              <div className="pt-6 border-t border-[#f3e7ea] dark:border-[#4a3b3e]">
                ...
                <h4 className="text-xs font-bold tracking-wider text-[#1b0e11] dark:text-white uppercase">
                    {t("컴플리먼터리 샘플", "Complimentary Sample")}
                  </h4>
                ...
                <div className="flex gap-3 overflow-x-auto pb-4 -mx-2 px-2 snap-x no-scrollbar">
                  {db.products.slice(0, 3).map((product) => {
                    const selected = activeSampleSlug === product.slug;
                    return (
                      <button
                        key={product.id}
                        type="button"
                      ...
                      >
                        ...
                        <p
                          className={cn(
                            ...
                            selected ? "text-[#e6194c]" : "text-[#1b0e11] dark:text-white group-hover:text-[#e6194c]",
                          )}
                        >
              ...
              <div className="space-y-3 mb-5">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-green-600 font-medium text-xs bg-green-50 px-2 py-0.5 rounded-full">
                    {t("결제 단계에서 계산", "Calculated at checkout")}
                  </span>
                </div>
              </div>
